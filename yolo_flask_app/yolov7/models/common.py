...
class RepConv_OREPA(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, padding=1, dilation=1, groups=1, padding_mode='zeros', deploy=False, use_se=False, nonlinear=nn.ELU()):
...
class SwinTransformerLayer(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=8, shift_size=0,
                 ...
                 act_layer=nn.ELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
class EPSABlock(nn.Module):
    def __init__(self, in_c, out_c):
        super().__init__()
        self.group_conv = nn.Conv2d(in_c, out_c, 1, groups=4)
    ...
    def forward(self, x):
        x_split = self.group_conv(x)
        weight = self.attn(x)
        return x_split * weight
...
class NAMBlock(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.spatial_attn = nn.Conv2d(in_c, in_c, 3, padding=1, groups=in_c)
        self.output_conv = nn.Conv2d(in_c, out_c, 1) if in_c != out_c else nn.Identity()

    def forward(self, x):
        c = self.channel_attn(x)
        s = torch.sigmoid(self.spatial_attn(x))
        x = x * c * s
...
class CG(nn.Module):
    def __init__(self, in_c, out_c, groups=2):
        super().__init__()
        self.block = nn.Sequential(
            nn.Conv2d(in_c, out_c, kernel_size=3, stride=1, padding=1, groups=groups),
            nn.BatchNorm2d(out_c),
            nn.GELU()
        )
